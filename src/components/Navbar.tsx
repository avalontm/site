...
import { FaCaretDown } from 'react-icons/fa'; // Para el ícono de la flecha

const Navbar: React.FC = () => {
  const { isAuthenticated, logout, user, role } = useAuth(); // Obtén el rol del contexto
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(false);
  const [isRegistroModalOpen, setIsRegistroModalOpen] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null); // Referencia para el menú desplegable
  const buttonRef = useRef<HTMLButtonElement>(null); // Referencia para el botón del menú
  ...
  // Cerrar el menú si el clic está fuera del menú o del botón del menú
  useEffect(() => {
      ...
      <nav className="sticky top-0 z-50 border-gray-200 bg-white dark:bg-gray-900">
        <div className="mx-auto flex max-w-screen-xl flex-wrap items-center justify-between p-4">
          <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
            ...
            <span className="self-center whitespace-nowrap text-2xl font-semibold dark:text-white">Partybara</span>
          </Link>
            ...
            {!isAuthenticated ? (
              <>
                <button
                  ...
                  className="rounded-lg px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-800 md:px-5 md:py-2.5"
                >
                  Iniciar sesión
                </button>
                <button
                  onClick={openRegistroModal}
                  className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 md:px-5 md:py-2.5"
                >
                  Registrarme
                </button>
              </>
                ...
                {isMenuOpen && (
                  <div ref={menuRef} className="absolute right-0 mt-2 w-48 rounded-md border border-gray-200 bg-white shadow-lg dark:bg-gray-800">
                    <ul>
                      ...
                      {role === 'admin' && ( // Verifica si el rol es 'admin'
                        <li>
                          <Link
                            to="/admin"
                            onClick={closeMenu}
                      ...
                      <li>
                        <button
                          ...
                          className="block w-full px-4 py-2 text-left text-sm text-gray-800 hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
                        >
                          Cerrar sesión
                        </button>
                      </li>
